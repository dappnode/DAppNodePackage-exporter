...
type ExecutionClient<T extends (typeof networks)[number]> = T extends "mainnet"
  ? (typeof executionClientsMainnet)[number]
  : T extends "prater"
  ? (typeof executionClientsPrater)[number]
  : T extends "gnosis"
  ? (typeof executionClientsGnosis)[number]
  : never;

type ConsensusClient<T extends (typeof networks)[number]> = T extends "mainnet"
  ? (typeof consensusClientsMainnet)[number]
  : T extends "prater"
  ? (typeof consensusClientsPrater)[number]
  : T extends "gnosis"
  ? (typeof consensusClientsGnosis)[number]
  : never;
...
function loadEnvs<T extends Network>(
  ...
  const executionClient = "geth.dnp.dappnode.eth";
  const consensusClient = "prysm.dnp.dappnode.eth";
